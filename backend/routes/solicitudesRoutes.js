...
router.post('/api/solicitudes/hospital', async (req, res) => {
  try {
    console.log("📥 Datos recibidos desde el hospital:", req.body); // Log para depuración
    ...
    const nuevaSolicitud = new Solicitud(req.body);
    const guardada = await nuevaSolicitud.save();
    console.log("✅ Solicitud guardada correctamente en MongoDB:", guardada); // Log para confirmar guardado
    res.json(guardada);
  } catch (err) {
    console.error("❌ Error al guardar solicitud en MongoDB:", err.message); // Log detallado del error
    res.status(500).json({ error: 'Error interno al procesar la solicitud en MongoDB' });
  }
});
